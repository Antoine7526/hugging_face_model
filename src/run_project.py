...
def ft_train(model, train_dataset, val_dataset, accelerator: str, devices: List[int]) -> pl.LightningModule:
    train_dataloader = DataLoader(train_dataset, batch_size=HP.batch_size, collate_fn=model.vectorizer.batch_collate_fn,
                                  shuffle=True)
    val_dataloader = DataLoader(val_dataset, batch_size=HP.batch_size, collate_fn=model.vectorizer.batch_collate_fn,
                                shuffle=False)
    # Define early stopping for the training
    early_stop = EarlyStopping(monitor=HP.es_monitor, mode=HP.es_mode, min_delta=HP.es_min_delta,
                               patience=HP.es_patience, verbose=True)
    ...
    # trainer
    trainer = pl.Trainer(max_epochs=HP.max_epochs, callbacks=[early_stop, checkpoint_callback], log_every_n_steps=10,
...
if __name__ == "__main__":
    ...
    argparser.add_argument('-r', '--nruns',
                           help='id (an integer) of the gpu to use. If not specified, model will run on cpu.', type=int,
    ...
    print()
    if testaccs:
        print("Mean Dev Acc.: %.2f (%.2f)\tMean Test Acc.: %.2f (%.2f)" % (
            np.mean(devaccs), np.std(devaccs), np.mean(testaccs), np.std(testaccs)))
    else:
